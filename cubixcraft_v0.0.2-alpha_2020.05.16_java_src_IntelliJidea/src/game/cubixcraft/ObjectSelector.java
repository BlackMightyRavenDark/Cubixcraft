...
public class ObjectSelector {
     ...
     */
    private void renderGhostCubeSide(Tessellator t, int x, int y, int z, int side) {
        float x0 = (float)x;
        float y0 = (float)y;
        float z0 = (float)z;
        float x1;
        ...
        for (int x = x0; x <= x1; x++) {
            for (int y = y0; y <= y1; y++) {
                for (int z = z0; z <= z1; z++) {
                     ...
                     */
                    if (player.worldObj.getBlockId(x, y, z) != BLOCK_AIR_ID &&
    ...
    public int pickObject(float radius) {
        ...
        if (hitsCount > 0) {
            ...
            Block block = player.worldObj.getBlock(cubeId);
