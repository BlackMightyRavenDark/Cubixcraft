...
public class Player {
     ...
     */
    public void reset() {
        setPosition(0.0f, worldObj.sizeZ + 2.0f, 0.0f);
    ...
    private void handleMouseMovement(float speed) {
        rotationYaw   += Mouse.getDX() * speed * mouseSensitivityCoef;
        rotationPitch -= Mouse.getDY() * speed * mouseSensitivityCoef;
    ...
    public void tick() {
        ...
        CubixCraft cubixCraft = CubixCraft.getCubixcraft();

        handleMovement((float)(walkingSpeed * speedCoef * cubixCraft.getGameTimer().getDeltaTime()));

        //симуляция гравитации
        motionY -= (float)(gravityY * gravityCoef * cubixCraft.getGameTimer().getDeltaTime());

        move(motionX, (float)(motionY * cubixCraft.getGameTimer().getDeltaTime()), motionZ);
