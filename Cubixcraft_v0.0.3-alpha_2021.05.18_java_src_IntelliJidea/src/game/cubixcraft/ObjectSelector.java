...
public class ObjectSelector {
    ...
    public ObjectSelector(Player player) {
        this.player = player;
     ...
     */
    public void renderHitCubeSide(Tessellator t) {
     ...
     */
    private void renderGhostCubeSide(Tessellator t, int x, int y, int z, int sideId) {
        float x0 = (float)x;
        float y0 = (float)y;
        float z0 = (float)z;
        float x1;
    ...
    private void renderGhostCubeSides(float r) {
         ...
         * Для этого раздвигаем область столкновения игрока во все стороны.
        ...
        for (int x = x0; x <= x1; x++) {
            for (int y = y0; y <= y1; y++) {
                for (int z = z0; z <= z1; z++) {
                     ...
                     * то добавляем его ID в буфер выбора и рисуем невидимые грани.
                     ...
                     */
                    if (player.world.getBlockId(x, y, z) != BLOCK_AIR_ID &&
                            frustumObj.cubeInFrustum((float)x, (float)y, (float)z,
                                    player.world.cubeSize)) {
                        /*
    ...
    public int pickObject(float radius) {
        ...
        gluPickMatrix((float)CubixCraft.getWindowWidth() / 2.0f, (float)CubixCraft.getWindowHeight() / 2.0f,
                2.0f, 2.0f, viewportBuffer);
        gluPerspective(45.0f, (float)CubixCraft.getWindowWidth() / (float)CubixCraft.getWindowHeight(),
                0.1f, 200.0f);
        ...
        if (hitsCount > 0) {
            ...
            Block block = player.world.getBlock(cubeId);
