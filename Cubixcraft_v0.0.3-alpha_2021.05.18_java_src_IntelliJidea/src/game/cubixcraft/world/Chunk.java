 ...
 * Если рендерить каждый блок по-отдельности (или все сразу), то будут жуткие тормоза.
 */
...
public class Chunk {
    ...
    //Нужен для определения количества обновлений чанков в секунду.
    ...
    /*
     * Экземпляр класса мира, в котором находится чанк.
     * Нужен для того, чтобы чанк имел доступ к необходимым параметрам мира,
     ...
     * Проверка должна производиться перед выполнением данного метода.
        ...
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                for (int z = z0; z < z1; z++) {
                    ...
                    if (blockId != Block.BLOCK_AIR_ID) {
                        Block block = world.createBlock(blockId);
                        block.setPosition(x, y, z);

                        int sidesRendered = block.render(tessellator);
