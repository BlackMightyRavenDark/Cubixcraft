...
{
    fontTex = SOIL_load_OGL_texture((char*)textureFileName.c_str(),
 ...
 * и переключает OpenGL в 2D-режим рисования.
 */
void glTextStart() 
{
...
void glTextOut(int xPos, int yPos, const char* sText, int color) 
{
    ...
    glColor3f(r, g, b);
    glTranslated((double)xPos, (double)(glScreenHeight - 20.0 - yPos), 0.0);
...
void DrawTextBkg(int xPos, int yPos, int xWidth, int color) 
{
    ...
    glLoadIdentity();
    glBegin(GL_QUADS);
    glVertex2f((GLfloat)xPos,            (GLfloat)(glScreenHeight - yPos - 20.0f));
    glVertex2f((GLfloat)(xPos + xWidth), (GLfloat)(glScreenHeight - yPos - 20.0f));
    glVertex2f((GLfloat)(xPos + xWidth), (GLfloat)(glScreenHeight - yPos)        );
    glVertex2f((GLfloat)xPos,            (GLfloat)(glScreenHeight - yPos)        );
    glEnd();
    glDisable(GL_BLEND);
}
