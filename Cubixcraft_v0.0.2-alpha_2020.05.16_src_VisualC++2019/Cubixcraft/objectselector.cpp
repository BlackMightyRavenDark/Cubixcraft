 ...
 */
void CObjectSelector::RenderGhostCubeSide(CTessellator* t, int x, int y, int z, int side) 
{
    float x0 = (float)x;
    float y0 = (float)y;
    float z0 = (float)z;
...
void CObjectSelector::RenderGhostCubeSides(float r) 
{
     ...
     * Для этого расширяем область столкновения игрока во все стороны.
 ...
 * !=0:
 * Игрок смотрит на блок. Информация о блоке передана в идентификатор структуры HITRESULT.
 * 0:
...
int CObjectSelector::PickObject(float radius) 
{
     ...
     *      обратиться к главному массиву в классе CWorld и получить полную
     *      информацию о блоке.
    ...
    if (hitsCount) 
    {
        ...
        CBlock* block = player->worldObj->GetBlock(cubeId);
...
std::string CObjectSelector::SideIdToString(int side) 
{
    switch (side) 
    {
        case BLOCK_SIDE_FRONT:
